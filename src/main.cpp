...
// BEGIN CONFIG
constexpr std::array POWER_PINS{25, 26, 27};
constexpr uint32_t DUT_SERIAL_RX = 19;
constexpr uint32_t DUT_SERIAL_TX = 18;
// END CONFIG

namespace {

template <typename T, size_t SIZE>
constexpr T getFlagsFromPins(const std::array<T, SIZE> &pins) {
  T flags = 0;
...
constexpr uint32_t POWER_PIN_MASK = getFlagsFromPins(POWER_PINS);
}  // namespace
...
void IRAM_ATTR setup() {
  for (size_t i = 0; i < POWER_PINS.size(); i++) {
